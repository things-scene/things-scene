...
describe('typescript project references', () => {
  ...
  const ThingsSceneReferences = references.map(prefixPackageName)
  ...
  references.map(packageName => {
    ...
    describe(`${displayedName}`, () => {
      it(`includes internal packages used as references`, () => {
        ...
        const ThingsScenePackage = dependencies.filter(lib => ThingsSceneReferences.includes(lib)).sort()

        expect(internalPackages).toStrictEqual(ThingsScenePackage)
      })
    })
  })
})
...
function prefixPackageName(packageName: string) {
  return `@things-scene-sdk/${packageName}`
}
